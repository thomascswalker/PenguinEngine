...
bool PViewport::ProjectWorldToScreen(const FVector3& WorldPosition, FVector3& ScreenPosition) const
{
	...
	if (Result.W > 0.0f)
	{
		...
		const FVector3 ClipPosition{Result.X / Result.W, Result.Y / Result.W, Result.Z / Result.W};
		...
		ScreenPosition = FVector3{NormalizedPosition.X * static_cast<float>(Camera->Width),
			NormalizedPosition.Y * static_cast<float>(Camera->Height), (Result.Z + 1.0f) * 0.5f};
		return true;
	}
...
void PViewport::FormatDebugText()
{
	...
	DebugText = std::format(
		...
		"FPS: {}\n"
		"Size: {}\n",
		Engine->GetFps(), GetSize().ToString());
}
