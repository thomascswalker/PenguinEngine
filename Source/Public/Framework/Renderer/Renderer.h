...
struct PChannel
{
	...
	PChannel(EChannelType InType, int32 InWidth, int32 InHeight)
		: Type(InType)
	{
	...
	constexpr int32 GetOffset(const int32 X, const int32 Y) const
	{
		return (Y * (Pitch / ChannelCount)) + X;
	}
	constexpr int32 GetPixelCount() const
	{
		return Width * Height;
	}
	constexpr int32 GetMemorySize() const
	{
		return Width * Height * BYTES_PER_PIXEL * ChannelCount;
	}
...
struct PTriangleRenderData
{
	float* DepthMemory;
	int32* ColorMemory;
	FVector3* ScreenPoints;
	float* DepthPoints;
	FRect Bounds;
	float OneOverArea;

	PTriangleRenderData(float* InDepthMemory, int32* InColorMemory, FVector3 InScreenPoints[], float InDepthPoints[],
		const FRect& InBounds = FRect(), const float InOneOverArea = 1.0f)
	{
		DepthMemory = InDepthMemory;
		ColorMemory = InColorMemory;
		Bounds = InBounds;
		OneOverArea = InOneOverArea;
		ScreenPoints = InScreenPoints;
		DepthPoints = InDepthPoints;
	}
};
...
class PRenderer
{
	...
	int32 ThreadCount = 1;
	std::atomic<int32> ThreadsComplete = 0;
...
public:
	...
	{
		return Viewport->GetCamera()->Width;
	}
	int32 GetHeight() const
	{
		return Viewport->GetCamera()->Height;
	}
	PViewport* GetViewport() const
	{
		return Viewport.get();
	}
	...
	std::shared_ptr<PChannel> GetColorChannel() const
	{
		return Channels.at("Color");
	}
	std::shared_ptr<PChannel> GetDepthChannel() const
	{
		return Channels.at("Depth");
	}
	...
	void DrawGrid() const;
	void Draw();
	void RasterizeTriangle(const PTriangleRenderData& RenderData);
